(function (ext) {
    ...
    ext.Setting_targetIP = function (ip, callback) {
        ...
        if (setupFlag_init == true) {
            ...
            flagArray.data.push({
                ...
                color_sensor_flag_yellow: false,
                ...
                success: function (data) {
                    ...
                    if (data.split(",")[2]) {
                        ...
                        switch (data.split(",")[2]) {
                            ...
                            case 'detectPerson':
                                console.log('detect person');
                                console.log(ip + " " + flagIndex + " " + "detect_person_flag true");
                                flagArray.data[flagIndex].detect_person_flag = true;
                                flagArray.data[flagIndex].index = data.split(",")[1];
                                console.log('index:' + flagArray.data[flagIndex].index);
                                break;
    ...
    ext.headMovement = function (ip, p1, p2, p3, p4, callback) {
        ...
        for (var j = 0; j < translate.headUDDegree.length; j++) {
            if (p4 == translate.headUDDegree[j]) {
                // Because the lowest degree of Zenbo head is -15 degree
        ...
        console.log("headMovement ip:" + ip + " p1:" + p1 + " p2:" + p2 + " p3:" + p3 + " p4:" + p4);
        $.ajax({
            url: 'http://' + ip + port + '/?extension=advance' + '&name=headMovement' + '&p1=' + p1 + '&p2=' + p2 + '&p3=' + p3 + '&p4=' + p4,
            dataType: 'text',
            crossDomain: true,
            success: function (data) {
                console.log("success handler");
    ...
    ext.when_read_color_and_run = function (ip, p1) {
        //console.log('block function: when_read_color_and_run()');
        for (var s = 0; s < translate.color_type.length; s++) {
            if (p1 == translate.color_type[s]) {
                p1 = TRANSLATIONS.us.color_type[s];
            }
        }
        ...
        //console.log('when_read_color_and_run: p1 = ' + p1);

        switch (p1) {
            case 'black':
            case '黑':
                if (flagArray.data[valueIndex].color_sensor_flag_black === true) {
                    flagArray.data[valueIndex].color_sensor_flag_black = false;
                    console.log('when_read_color_and_run: case: black');
                    return true;
                }
                break;
            ...
            case 'blue':
            case '藍':
                if (flagArray.data[valueIndex].color_sensor_flag_blue === true) {
                    flagArray.data[valueIndex].color_sensor_flag_blue = false;
                    console.log('when_read_color_and_run: case: blue');
                    return true;
                }
                break;
            ...
            case 'green':
            case '綠':
                if (flagArray.data[valueIndex].color_sensor_flag_green === true) {
                    flagArray.data[valueIndex].color_sensor_flag_green = false;
                    console.log('when_read_color_and_run: case: green');
                    return true;
                }
                break;
            ...
            case 'red':
            case '紅':
                if (flagArray.data[valueIndex].color_sensor_flag_red === true) {
                    flagArray.data[valueIndex].color_sensor_flag_red = false;
                    console.log('when_read_color_and_run: case: red');
                    return true;
                }
                break;
            ...
            case 'white':
            case '白':
                if (flagArray.data[valueIndex].color_sensor_flag_white === true) {
                    flagArray.data[valueIndex].color_sensor_flag_white = false;
                    console.log('when_read_color_and_run: case: white');
                    return true;
                }
                break;
            ...
            case 'yellow':
            case '黃':
                if (flagArray.data[valueIndex].color_sensor_flag_yellow === true) {
                    flagArray.data[valueIndex].color_sensor_flag_yellow = false;
                    console.log('when_read_color_and_run: case: yellow');
                    return true;
                }
                break;
        }
    ...
    ext.followLineRun = function (ip, p1) {

        for (var i = 0; i < translate.MenuFollowLineStartStop.length; i++) {
            if (p1 == translate.MenuFollowLineStartStop[i]) {
                p1 = TRANSLATIONS.us.MenuFollowLineStartStop[i];
            }
        }

        console.log("followLineRun - " + "ip:" + ip + " p1:" + p1);
        $.ajax({
            url: 'http://' + ip + port + '/?extension=advance' + '&name=followLineRun' + '&p1=' + p1,
            dataType: 'text',
            crossDomain: true,
            success: function (data) {
                console.log("success handler");
    ...
    ext.followLineConfig = function (ip, p1, p2) {
        for (var i = 0; i < translate.MenuFollowLineStartStop.length; i++) {
            if (p1 == translate.MenuFollowLineStartStop[i]) {
                p1 = TRANSLATIONS.us.MenuFollowLineStartStop[i];
            }
        }
        for (var i = 0; i < translate.MenuFollowLineColorPatternngth; i++) {
            if (p2 == translate.MenuFollowLineStartStop[i]) {
                p2 = TRANSLATIONS.us.MenuFollowLineStartStop[i];
            }
        }

        console.log("ext: followLineConfig: " + "ip: " + ip + ", p1:" + p1);
        $.ajax({
            url: 'http://' + ip + port + '/?extension=advance' + '&name=followLineConfig' + '&p1=' + p1 + '&p2=' + p2,
            dataType: 'text',
            crossDomain: true,
            success: function (data) {
                console.log("success handler");
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                console.log("error handler");
                if (zenboIPWarningWindsowFlag === true) showAlertMessage();
            }
        });
    };

    ext.startReadColor = function (ip, p1) {
        for (var i = 0; i < translate.MenuReadColor.length; i++) {
            if (p1 == translate.MenuReadColor[i]) {
                p1 = TRANSLATIONS.us.MenuReadColor[i];
            }
        }

        console.log("startReadColor");
        console.log("startReadColor - " + "ip:" + ip + " p1:" + p1);
        $.ajax({
            url: 'http://' + ip + port + '/?extension=advance' + '&name=startReadColor' + '&p1=' + p1,
            dataType: 'text',
            crossDomain: true,
            success: function (data) {
                console.log("success handler");
    ...
    ext.disableVoiceTrigger = function (ip, p1) {
        for (var i = 0; i < translate.MenuDisableVoiceTrigger.length; i++) {
            if (p1 == translate.MenuDisableVoiceTrigger[i]) {
                p1 = TRANSLATIONS.us.MenuDisableVoiceTrigger[i];
            }
        }

        console.log("disableVoiceTrigger - " + "ip:" + ip + " p1:" + p1);
        $.ajax({
            url: 'http://' + ip + port + '/?extension=advance' + '&name=disableVoiceTrigger' + '&p1=' + p1,
            dataType: 'text',
            crossDomain: true,
            success: function (data) {
                console.log("success handler");
    ...
    ext.stopListening = function (ip) {
        console.log("stopListening - " + "ip:" + ip);
        $.ajax({
            url: 'http://' + ip + port + '/?extension=advance' + '&name=stopListening',
            dataType: 'text',
            crossDomain: true,
            success: function (data) {
                console.log("success handler");
    ...
    ext.detectPerson = function (ip, p1) {
        for (var i = 0; i < translate.detectPerson.length; i++) {
            if (p1 == translate.detectPerson[i]) {
                p1 = TRANSLATIONS.us.detectPerson[i];
            }
        }

        console.log("detectPerson - " + "ip:" + ip + " p1:" + p1);
        $.ajax({
            url: 'http://' + ip + port + '/?extension=advance' + '&name=detectPerson' + '&p1=' + p1,
            dataType: 'text',
            crossDomain: true,
            success: function (data) {
                console.log("success handler");
    ...
    ext.when_detected_person_and_run = function (ip) {

        var valueIndex_detected_person = getValueIndex(ip);

        for (var j = 0; j < flagArray.data.length; j++) {

            if (ip == flagArray.data[j].device) {
                valueIndex_detected_person = j;
            }
        }

        if (valueIndex_detected_person === -1) {

            console.log('valueIndex_detected_person === -1');
            return false;
        }

        if (flagArray.data[valueIndex_detected_person].detect_person_flag === true) {
            console.log('true detected person');
            flagArray.data[valueIndex_detected_person].detect_person_flag = false;
            return true;
        }
    ...
    ext.faceToPerson = function (ip, p1) {
        for (var i = 0; i < translate.faceToPersonItems.length; i++) {
            if (p1 == translate.faceToPersonItems[i]) {
                p1 = TRANSLATIONS.us.faceToPersonItems[i];
            }
        }

        console.log("faceToPerson - " + "ip:" + ip + " p1:" + p1);
        $.ajax({
            url: 'http://' + ip + port + '/?extension=advance' + '&name=faceToPerson' + '&p1=' + p1,
            dataType: 'text',
            crossDomain: true,
            success: function (data) {
                console.log("success handler");
    ...
    var TRANSLATIONS = {
        us: {
            ...
            startWheelLight: 'IP %s start Wheel light with %m.wheelLightLights wheel %m.wheelLightSpeed color %m.startWheelLightColor and effect %m.startWheelLightEffect',
            stopWheelLight: 'IP %s stop Wheel light %m.wheelLightLights wheel',
            headMovement: 'IP %s turn head to the %m.headLRDirection by %m.headLRDegree degree(s)  %m.headUDDirection by %m.headUDDegree degree(s)',
            followLineRun: 'IP %s %m.MenuFollowLineStartStop start follow line',
            followLineConfig: 'IP %s update follow line behavior, when see %m.MenuFollowLineColorPattern do %m.MenuFollowLineBehavior',
            startReadColor: 'IP %s %m.MenuReadColor read color',
            when_read_color_and_run: 'when Zenbo IP %s  read color %m.color_type',
            disableVoiceTrigger: 'IP %s %m.MenuDisableVoiceTrigger listening hey, Zenbo',
            stopListening: 'IP %s stop listening',
            detectPerson: '%m.detectPersonItems detected person(s)',
            when_detected_person_and_run: 'when detected person(s)',
            faceToPerson: 'IP %s face to %m.faceToPersonItems person',
            ...
            alertButtonText: 'OK',
            MenuFollowLineStartStop: ["start", "end"],
            MenuFollowLineColorPattern: ["black", "blue", "green", "red", "white", "yellow"],
            MenuFollowLineBehavior: ["CROSSROAD_LEFT", "CROSSROAD_RIGHT", "CURVE", "FORKED_LEFT", "FORKED_RIGHT", "NOTHING", "ROTATION", "SPEED_LEVEL_SLOW", "SPEED_LEVEL_NORMAL", "SPEED_LEVEL_FAST", "TERMINATE", "U_TURN"],
            color_type: ["black", "blue", "green", "red", "white", "yellow"],
            MenuReadColor: ["start", "stop"],
            MenuDisableVoiceTrigger: ["disable", "enable"],
            detectPersonItems: ["start", "stop"],
            faceToPersonItems: ["seen"],
        },
        tw: {
            ...
            followLineRun: 'IP %s %m.MenuFollowLineStartStop 沿線行走',
            followLineConfig: 'IP %s 設定沿線行走行為, 當看到 %m.MenuFollowLineColorPattern 做 %m.MenuFollowLineBehavior ',
            startReadColor: 'IP %s %m.MenuReadColor 讀取顏色',
            when_read_color_and_run: '當 Zenbo IP %s 讀取到顏色 %m.color_type',
            faceToPerson: 'IP %s 面向 %m.faceToPersonItems 人',
            disableVoiceTrigger: 'IP %s %m.MenuDisableVoiceTrigger 聽 hey, Zenbo',
            stopListening: 'IP %s 停止聽話',
            detectPerson: 'IP %s %m.detectPersonItems 偵測人',
            when_detected_person_and_run: '當 IP %s 偵測到人',
            ...
            alertButtonText: '確定',
            MenuFollowLineStartStop: ["開始", "停止"],
            MenuFollowLineColorPattern: ["黑", "藍", "綠", "紅", "白", "黃"],
            MenuFollowLineBehavior: ["CROSSROAD_LEFT", "CROSSROAD_RIGHT", "CURVE", "FORKED_LEFT", "FORKED_RIGHT", "NOTHING", "ROTATION", "SPEED_LEVEL_SLOW", "SPEED_LEVEL_NORMAL", "SPEED_LEVEL_FAST", "TERMINATE", "U_TURN"],
            color_type: ["黑", "藍", "綠", "紅", "白", "黃"],
            MenuReadColor: ["開始", "停止"],
            MenuDisableVoiceTrigger: ["停止", "開始"],
            detectPersonItems: ["開始", "關閉"],
            faceToPersonItems: ["看到的"],
        },
    }
    ...
    var descriptor = {
        blocks: [
            ['w', translate.Setting_targetIP, 'Setting_targetIP', "ip"],
            ['', translate.Body_movement, 'Body_movement', "ip", translate.move_direction[0], translate.move_far[0], translate.move_speed[0]],
            ['', translate.Stop_moving, 'Stop_moving', "ip"],
            ['w', translate.headMovement, 'headMovement', "ip", translate.headLRDirection[0], translate.headLRDegree[0], translate.headUDDirection[0], translate.headUDDegree[1]],
            ['', translate.Head_movement, 'Head_movement', "ip", translate.head_direction[2], translate.head_degree[1]],
            ['', translate.Body_turn, 'Body_turn', "ip", translate.body_turn_direction[0], translate.body_turn_degree[3]],
            ['', translate.Remote_control_body, 'Remote_control_body', "ip", translate.remote_control_body[3]],
            //  ['', translate.Action, 'Action', "ip", translate.action_type[3]],
            //  ['', translate.Cancel_actionset, 'Cancel_actionset', "ip"],
            ['', translate.Facial, 'Facial', "ip", translate.facial_type[5]],
            ['', translate.hideFace, 'hideFace', "ip"],
            ['', translate.TTS, 'TTS', "ip", translate.tts_type[0]],
            ['', translate.TTS_editor, 'TTS_editor', "ip", translate.ttsEditorDefaultText],
            ['', translate.Adjust_stream_volume, 'Adjust_stream_volume', "ip", translate.volume_option_type[3], translate.volume_type[0]],
            ['', translate.Adjust_tts_and_speed, 'Adjust_tts_and_speed', "ip", translate.ttsEditorDefaultText, translate.tts_speed_type[1]],
            ['', translate.Add_and_update_sentence, 'Add_and_update_sentence', "ip", translate.sentence_type[0], translate.defaultAddedSentence],
            ['', translate.Speak_and_listen, 'Speak_and_listen', "ip"],
            ['h', translate.when_listen_and_run, 'when_listen_and_run', "ip", translate.second_sentence_type[0]],
            ['', translate.Delete_instance, 'Delete_instance', "ip"],
            //  ['', translate.Add_and_update_sentence_number, 'Add_and_update_sentence_number', "ip"],
            ['h', translate.when_listen_number_and_run, 'when_listen_number_and_run', "ip"],
            ['r', translate.getIndex, 'getIndex', "ip"],
            ['', translate.playVideosInYoutube, 'playVideosInYoutube', "ip", translate.playVideosInYoutubeItems[0], 'https://www.youtube.com/watch?v=Ou21RusvBcg'],
            ['', translate.playUrlMusic, 'playUrlMusic', "ip", translate.playUrlMusicItems[0], 'https://zenboscratch.github.io/examples/zenbo_music.mp3'],
            ['', translate.displayUrlPictures, 'displayUrlPictures', "ip", translate.displayUrlPicturesItems[0], 'https://zenboscratch.github.io/images/zenbo.jpg'],
            ['', translate.openDriveVideoUrl, 'openDriveVideoUrl', "ip", translate.openDriveVideoUrlItems[0], 'https://drive.google.com/open?id=0B5o6VwYT7NaiUUhrTW12Ql9tcUk'],
            ['', translate.openDriveAudioUrl, 'openDriveAudioUrl', "ip", translate.openDriveAudioUrlItems[0], 'https://drive.google.com/open?id=0B5o6VwYT7NaibHJ0LWtHN0JtVFU'],
            ['', translate.openDrivePictureUrl, 'openDrivePictureUrl', "ip", translate.openDrivePictureUrlItems[0], 'https://drive.google.com/open?id=0B5o6VwYT7NaiSVJ0S3JKeEZwODA'],
            ['', translate.openDriveDocumentUrl, 'openDriveDocumentUrl', "ip", translate.openDriveDocumentUrlItems[0], 'https://drive.google.com/open?id=0B5o6VwYT7NaiN1h3SXZHTjRsc2s'],
            ['h', translate.when_touch_head_and_run, 'when_touch_head_and_run', "ip"],
            ['', translate.recordAudio, 'recordAudio', "ip", translate.recordAudioItems[0], 'testAudio'],
            ['', translate.recordVideo, 'recordVideo', "ip", translate.recordVideoItems[0], translate.recordVideoSizes[0], 'testVideo'],
            ['', translate.startWheelLight, 'startWheelLight', "ip", translate.wheelLightLights[0], translate.wheelLightSpeed[2], translate.startWheelLightColor[5], translate.startWheelLightEffect[3]],
            ['', translate.stopWheelLight, 'stopWheelLight', "ip", translate.wheelLightLights[0]],
            // ZenboJr only ['', translate.followLineRun, 'followLineRun', "ip",  translate.MenuFollowLineStartStop[0]],
            // ZenboJr only ['', translate.followLineConfig, 'followLineConfig', "ip",  translate.MenuFollowLineColorPattern[0], translate.MenuFollowLineBehavior[0]],
            // ZenboJr only ['', translate.startReadColor, 'startReadColor', "ip", translate.MenuReadColor[0]],
            // ZenboJr only ['h', translate.when_read_color_and_run, 'when_read_color_and_run',  "ip", translate.color_type[0]],
            ['', translate.disableVoiceTrigger, 'disableVoiceTrigger', "ip", translate.MenuDisableVoiceTrigger[0]],
            ['', translate.stopListening, 'stopListening', "ip"],
            ['', translate.detectPerson, 'detectPerson', "ip", translate.detectPersonItems[0]],
            ['h', translate.when_detected_person_and_run, 'when_detected_person_and_run', "ip"],
            ['', translate.faceToPerson, 'faceToPerson', "ip", translate.faceToPersonItems[0]],
        ],
        menus: {
            ...
            // ZenboJr only "MenuFollowLineStartStop": translate.MenuFollowLineStartStop,
            // ZenboJr only "MenuFollowLineColorPattern": translate.MenuFollowLineColorPattern,
            // ZenboJr only "MenuFollowLineBehavior": translate.MenuFollowLineBehavior,
            // ZenboJr only "color_type": translate.color_type,
            // ZenboJr only "MenuReadColor": translate.MenuReadColor,
            "MenuDisableVoiceTrigger": translate.MenuDisableVoiceTrigger,
            "detectPersonItems": translate.detectPersonItems,
            "faceToPersonItems": translate.faceToPersonItems,
        },
...
(function (ext) { var flagarray = { data: [] }; port = ":8080"; extensionv
（功能（擴展名）{
